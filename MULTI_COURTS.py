...
TRACKING_FILE = "court_tracking.json"
...
def solve_canlii_audio_captcha(page):
	...
	try:
		...
		if audio_toggle.count() == 0:
			print("    ⚠️  Audio toggle button not found")
			return False
			
		print("    Clicking audio toggle button...")
		try:
			audio_toggle.click()
			page.wait_for_timeout(1000)
		except Exception as e:
			print(f"    ⚠️  Could not click audio toggle: {e}")
			return False
			
		# Get audio source
		audio_tag = page.locator("#audioCaptchaTag")
		if audio_tag.count() == 0:
			print("    ⚠️  Audio tag not found")
			return False
			
		audio_src = audio_tag.get_attribute("src")
		if not audio_src:
			print("    ⚠️  Audio source not found")
			return False
			
		full_audio_url = BASE_URL + audio_src if audio_src.startswith("/") else audio_src
		print(f"    📥 Downloading audio from: {full_audio_url[:50]}...")
		
		# Download audio
		cookies = get_cookies_dict(page)
		response = requests.get(full_audio_url, cookies=cookies, timeout=30)
		...
		print(f"    🔢 Transcribed text: {numbers}")
		...
		# Submit
		print("    Submitted answer, clicking ok...")
		page.locator("input[type='submit'][value='ok']").click()
		page.wait_for_timeout(3000)
		
		if not is_captcha_page(page):
			print("    ✅ CanLII Audio CAPTCHA solved successfully!")
			return True
		else:
			print("    ❌ CAPTCHA solution incorrect")
			return False
			
	except Exception as e:
		print(f"    ⚠️  Error solving CanLII audio CAPTCHA: {e}")
		return False
...
def solve_captcha_automatically(page):
	...
	for attempt in range(1, MAX_CAPTCHA_ATTEMPTS + 1):
		...
		print("    ⚠️  Audio solve failed/skipped, trying Visual/Bedrock...")
...
def mark_as_processed(tracking_data, doc_info):
	"""Mark a document as processed with detailed info and save safely"""
	# We rely on the passed-in tracking_data for in-memory checks, 
	# but we must ensure file writes are atomic.
	# Also, it's safer to re-read the file to ensure we don't overwrite other threads' work
	with file_lock:
		try:
			# Re-load from disk to get latest state
			current_data = load_tracking_data()
			if "processed_documents" not in current_data:
				current_data["processed_documents"] = []
			
			# Check against the fresh data
			if not is_already_processed(current_data, doc_info.get("url")):
				current_data["processed_documents"].append(doc_info)
				save_tracking_data(current_data)
				
				# Update the local cache as well (optional but good for consistency)
...
def process_year_page(page, year_url, court_name, year, tracking_data):
	"""Process a specific year page for a court"""
	try:
		...
		save_dir = os.path.join("downloads", court_name, year)
...
def process_court(page, court_url, tracking_data):
	"""Process a court main page"""
	try:
		print(f"\nProcessing Court: {court_url}")
		...
		if more_link.count() > 0:
			...
			print("  ℹ️ 'more, by year' link not found, checking current page for years...")

		# Check for years selector (either on new page or current page)
		years_found = False
		try:
			if page.locator("#navYearsSelector").count() > 0:
				years_found = True
			else:
				# Try waiting briefly if it might be loading
				try:
					page.wait_for_selector("#navYearsSelector", timeout=3000)
					years_found = True
				except:
					pass
		except:
		...
		if years_found:
			...
			court_name = court_url.rstrip("/").split("/")[-1]
			
			# Process each year
			for year_text, year_val in years_data:
				process_year_page(page, year_val, court_name, year_text, tracking_data)
		else:
			print("  ⚠️ Years selector not found on this page")
...
def create_page(p):
	"""Create a configured browser, context, and page"""
...
def process_court_worker(url, tracking_data):
	"""Worker function to process a court with its own browser"""
	print(f"🚀 Starting worker for: {url}")
	try:
		with sync_playwright() as p:
			browser, context, page = create_page(p)
			try:
				process_court(page, url, tracking_data)
			finally:
				browser.close()
	except Exception as e:
...
def main():
	...
	# 1. Collect links using a single browser instance
	...
	with sync_playwright() as p:
		...
		try:
			...
			if datadome_detected or canlii_detected or is_captcha_page(page):
				...
				try:
					page.wait_for_selector(f"h2:has-text('{SECTION_TITLE}')", timeout=10000)
				except:
			...
			for attempt in range(1, MAX_RETRIES + 1):
				...
				print(f"⚠️  No links found (Attempt {attempt}/{MAX_RETRIES}). Checking for CAPTCHA...")
				...
				try:
					...
					print(f"    Warning: Navigation/Reload error: {e}")
				
				page.wait_for_timeout(WAIT_MS)
				links = collect_links(page, SECTION_TITLE)
				print(f"Found {len(links)} links after retry {attempt}")
	...
	# 2. Process links in parallel using thread pool
	print(f"\n=== Phase 2: Processing {len(links)} Links in Parallel ===")
	
	# Adjust max_workers based on your system capability
	MAX_WORKERS = 3 
	
	full_urls = [f"{BASE_URL}{href}" for href in links]
	
	with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
		# Submit all tasks
		future_to_url = {executor.submit(process_court_worker, url, tracking_data): url for url in full_urls}
